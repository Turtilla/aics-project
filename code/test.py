...
class CaptionTester:
    ...
    def visualize_att(self, image_path, seq, alphas, smooth=False):
        """
        ...
        for index, word in enumerate(words):
            if index > 50:
                break
            plt.subplot(int(np.ceil(len(words) / 5.)), 5, index + 1)

            plt.text(0, 1, '%s' % (word), color='black', backgroundcolor='white', fontsize=12)
            plt.imshow(image)
            current_alpha = alphas[index, :]
    ...
    def test_images(self, img, beam_size, smooth):
        # Encode, decode with attention and beam search
        seq, alphas = self.caption_image_beam_search(img, beam_size)
        alphas = torch.FloatTensor(alphas)

        # Visualize caption and attention of best sequence
        self.visualize_att(img, seq, alphas, smooth)
